...
Each card carries three caption lines: `.card__title`, `.card__meta` (the artist
credit) and `.card__date`. The date is a `<time>` element, so keep the
machine-readable `datetime="YYYY-MM-DD"` in sync with the text. Releases are
listed newest first.
